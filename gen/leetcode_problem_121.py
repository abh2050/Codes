...
class Solution:
    def isWithinRange(self, num: int, lower: int, upper: int) -> bool:
        ...
        Returns:
            True if num is within the range, False otherwise.
        """
        # Simple comparison to check if num is within the bounds
        return lower <= num <= upper


# Time Complexity: O(1) - Constant time, as it involves simple comparisons.
# Space Complexity: O(1) - Constant space, as no extra space is used.
...
print(solution.isWithinRange(1000, -1000, 1000)) # Output: True
